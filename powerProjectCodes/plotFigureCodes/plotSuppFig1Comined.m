...
% close all
clear
supFigure1 = figure('WindowState','maximized');
fh.WindowState = 'maximized';

h1 = axes('Parent',supFigure1,'Position',[0.076875+0.03 0.421185372005044 0.871875 0.433795712484237]);
...
%folderSourceString  = '\\10.120.10.130\NeoLabData\Projects\ProjectDhyaan\BK1';
% folderSourceString  = '\\NEOLABDATA\NeoLabData\Projects\ProjectDhyaan\BK1';
...
for i=1:numSubjects
dp{i} = '';
if ismember(i,[1 36 76])
dp{i} = i;
end
end
imagesc(1:numSubjects,numElecs:-1:1,flipud(sortedElecMatrix),'parent',h1);
set(h1,'Xtick',1:1:numSubjects,'XTickLabel',dp,'TickDir','out','TickLength',[0.005, 0.001],'XTickLabelRotation',0);
...
h0 = axes('Parent',supFigure1,'Position',[0.0781    0.4237    0.0244    0.4313]);
subplot(h0);
...
h2 = axes('Parent',supFigure1,'Position',[ 0.1069    0.8625    0.8719    0.0467]);
...
text('parent',h2,'String','Meditators','Color',colorNames(1,:),'Rotation',0,'FontSize',16,'fontweight','bold','Position',[15.24229390681 312.702702702703 0]);

text('String','Controls','Color',colorNames(2,:),'Rotation',0,'FontSize',16,'fontweight','bold','Position',[55.4487  312.1622  0],'parent',h2);
...
%%
addSupplementaryFig =1;
if addSupplementaryFig
    
    dataArray{1,1} = badElecPercentageIndMeditators * numElecs;
    dataArray{1,2} = badElecPercentageIndConrols * numElecs;

    badSubInd1Pos = find(badSubInd<length(dataArray{1,1}));
    badSubInd2 = badSubInd(badSubInd1Pos+1:end)-length(dataArray{1,1});

    dataArray{1,1}(badSubInd(badSubInd1Pos)) = [];
    dataArray{1,2}(badSubInd2) = [];
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%     h2d = axes('Parent',figure1,'Position', [0.7500 0.0773 0.0988 0.2228]);
    ...
    hPlot0 = getPlotHandles(1,4,[0.1 0.042 0.88 0.25],0.08);
    ...
    for iParam = 1:length(param)
        ...
        for iGender = 1:2
            ...
            if iParam == 3
                dataShift = [0 0];
                for iGroup = 1:numGroup
                data{iGroup}((data{iGroup}==0)) = nan;
                dataUP{iGroup}((dataUP{iGroup}==0)) = nan;
            end
            else
        ...
        xlim([0 3]);
        set(hPlot(1),'XTick',[1 2],'XTickLabel',[{strList{1}} {strList{2}}],'TickLength',[0.04 0.02],'LineWidth',1,'FontWeight','bold',FontSize=14);
       
        h = gca;
        diffYlim = h.YLim(1)-0;
        h.YLim = [0 h.YLim(2)+0.55*diffYlim];
        if iParam==4            
            h.YTickLabel = h.YTick;
             ylabel('No. of bad elecs (tot)',FontSize=14,FontWeight="bold");
             title('No. of bad elecs',FontSize=16,FontWeight="bold");
        else
        ...
        if pp<0.001
            text(0.25,pPosition,['p_P = ' num2str(pp,'%.1e')],'FontWeight','bold','FontSize',12);
        else
            text(0.25,pPosition,['p_P = ' num2str(pp,'%.2f')],'FontWeight','bold','FontSize',12);
        end

        if pu<0.001
            text(1.75,pPosition,['p_U = ' num2str(pu,'%.1e')],'FontWeight','bold','FontSize',12);
        else
            text(1.75,pPosition,['p_U = ' num2str(pu,'%.2f')],'FontWeight','bold','FontSize',12);
        end
...
set(findobj(supFigure1,'type','axes'),'box','off','FontSize',14....
    ,'FontWeight','Bold'...
    ,'TickDir','out'...
...
annotation(supFigure1,'textbox',[0.061875 0.90442244764625 0.0340624991711229 0.0542244628203321],'String',{'A'},'LineStyle','none','FontWeight','bold',...
    'FontSize',20);
annotation(supFigure1,'textbox',[0.061875 0.302909206788747 0.0340624991711229 0.0542244628203322],'String',{'B'},'LineStyle','none','FontWeight','bold',...
    'FontSize',20);
...
if saveFlag
    supFigure1.Color = [1 1 1];
    savefig(supFigure1,fullfile(finalPlotsSaveFolder,'MeditationFigureSupp1.fig'));
    print(supFigure1,fullfile(finalPlotsSaveFolder,'MeditationFigureSupp1'),'-dsvg','-r300');
    print(supFigure1,fullfile(finalPlotsSaveFolder,'MeditationFigureSupp1'),'-dtiff','-r300');
     print(supFigure1,fullfile(finalPlotsSaveFolder,'MeditationFigureSupp1'),'-dpdf','-r300');
    print(supFigure1,fullfile(finalPlotsSaveFolder,'MeditationFigureSupp1u'),'-dpdf','-bestfit','-r300');
end
