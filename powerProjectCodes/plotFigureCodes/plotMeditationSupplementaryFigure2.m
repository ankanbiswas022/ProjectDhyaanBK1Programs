...
saveFlag = 0;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Initial variables %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
supFigure2 = figure('WindowState','maximized','Color',[1 1 1]);
...
for i=1:numProtocols
    ...
    title(protocolNames{i},'fontsize',22);
    subtitle(['(' groupNames{1} '-' groupNames{2} ')'],'fontsize',22);
    if i==2
        ach=colorbar;
        ach.Location       ='southoutside';
        ach.Position       =  [0.4106    0.1866    0.1781    0.0404];  ach.FontWeight = 'bold'; ach.FontSize  =16;
        ach.Label.String   = 'Change in Power (dB)';
        ach.Label.FontSize = 18;
    end
end
...
if saveFlag
    supFigure2.Color = [1 1 1];
    savefig(supFigure2,fullfile(finalPlotsSaveFolder,'MeditationFigureSupp2.fig'));
    print(supFigure2,fullfile(finalPlotsSaveFolder,'MeditationFigureSupp2'),'-dsvg','-r300');
    print(supFigure2,fullfile(finalPlotsSaveFolder,'MeditationFigureSupp2'),'-dtiff','-r300');
    print(supFigure2,fullfile(finalPlotsSaveFolder,'MeditationFigureSupp2'),'-dpdf','-r300');
    print(supFigure2,fullfile(finalPlotsSaveFolder,'MeditationFigureSupp2'),'-dpdf','-bestfit','-r300');
end