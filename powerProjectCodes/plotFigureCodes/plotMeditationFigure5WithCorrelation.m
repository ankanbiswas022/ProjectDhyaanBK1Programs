...
protocolNames  = [{'M2'}    {'M2'}    {'M2'}  {'M1'}     {'G2'}     {'M1'}  {'G2'}  {'M2'}];
refChoices     = [{'none'}  {'none'}  {'M2'}  {'none'}   {'none'}   {'EO1'} {'G2'}  {'G2'}] ;
analysisChoice = [{'bl'      ,'st',    'st',  'combined',  'bl'}    {'combined'} {'st'}  {'bl'}];
...
hrawPSD = getPlotHandles(2,2,[0.1 0.12 0.28 0.8],0.02,0.05);
hdeltaPSD = getPlotHandles(2,1,[0.44 0.12 0.18 0.8],0.05);
hAllPlots = [hrawPSD hdeltaPSD];
hTopo     = getPlotHandles(4,1,[0.7 0.12 0.06 0.8],0.05,0.06);
hCorr     = getPlotHandles(2,1,[0.83 0.12 0.15 0.8],0.05,0.06);

annotation('textbox',[.115 .65 .1 .2], 'String','Occipital','EdgeColor','none','FontWeight','bold','FontSize',annotationFontSize,'Rotation',90,'Color',colorList(1,:));
annotation('textbox',[.115 .18 .1 .2], 'String','Fronto-Temporal','EdgeColor','none','FontWeight','bold','FontSize',annotationFontSize,'Rotation',90,'Color',colorList(2,:));
...
if displayInsetFlag
    for g=1:length(groupPos)
        for i=1:3
            ...
            if i==1 || i==2
                hInset = axes('position', [hPos(1)+0.06 hPos(2)+0.26  0.0563    0.1114]);
            else
                hInset = axes('position', [hPos(1)+0.1 hPos(2)+0.26  0.0563    0.1114]);
            end
...
% Correlation (linear dependence) between spontaneous stimulus free gamma and the stimulus-induced gamma
% two variables for each subjects

% get the spontaneous gamma from the spontanous perod 
% get the stimulus induced gamma from the stim period. 


%% get the data
% for M2 segment
gList=1:2;        % meditators
sList=[8 3];      % first two segmnets, first
eList=1;        % occipital
...
for e=eList
    for g=gList
        ...
        scatter(hCorr(e),dataToPlot(:,1),dataToPlot(:,2),30,displaySettings.colorNames(g,:),'filled','o');
        hold(hCorr(e),'on');

        % linear fit of the line
        linearCoefficients = polyfit(dataToPlot(:,1), dataToPlot(:,2), 1);
        xFit = linspace(min(dataToPlot(:,1)), max(dataToPlot(:,1)), 100);
        yFit = polyval(linearCoefficients, xFit);
        plot(hCorr(e),xFit, yFit,'Color', displaySettings.colorNames(g,:),'MarkerSize', 15,'LineWidth', 2);
        slope = linearCoefficients(1);
        % add r and p-value in the plots
    end
end
...
for i=1:2
    if i==2
        xlabel(hCorr(i),'Spontanous Gamma Power','FontWeight','bold','FontSize',labelFontSize);
    end
    ylabel(hCorr(i),'Stim-Induced Gamma Power','FontWeight','bold','FontSize',labelFontSize);
end
...
annotation(gcf,'textbox',[   0.1881    0.7415    0.0738    0.0381],'Color',[0.768627450980392 0.0862745098039216 0.941176470588235],...
    'String',{'Meditators'},...
    'LineStyle','none',...
    'FontWeight','bold',...
    'FontSize',14,...
...
annotation(gcf,'textbox',[    0.1883    0.7124    0.0986    0.0394],'Color',[0.250980392156863 0.411764705882353 0.87843137254902],...
    'String','Controls',...
    'LineStyle','none',...
    'FontWeight','bold',...
    'FontSize',14,...
    'FitBoxToText','off');
annotation(gcf,'textbox',[   0.1776    0.6809    0.0986    0.0394],'Color',[0.501960784313725 0 0.501960784313725],...
    'String','-- Med (M1)',...
    'LineStyle','none',...
    'FontWeight','bold',...
    'FontSize',14,...
...
annotation(gcf,'textbox',[    0.1775    0.6545    0.0975    0.0381],'Color',[0 0 1], ...
    'String','-- Con (M1)',...
    'LineStyle','none',...
    'FontWeight','bold',...
    'FontSize',14,...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%For 2nd plot
annotation(gcf,'textbox',[   0.3387    0.7415    0.0738    0.0381],'Color',[0.768627450980392 0.0862745098039216 0.941176470588235],...
    'String',{'Meditators'},...
    'LineStyle','none',...
    'FontWeight','bold',...
    'FontSize',14,...
...
annotation(gcf,'textbox',[   0.3381    0.7150    0.0737    0.0381],'Color',[0.250980392156863 0.411764705882353 0.87843137254902],...
    'String','Controls',...
    'LineStyle','none',...
    'FontWeight','bold',...
    'FontSize',14,...
    'FitBoxToText','off');
annotation(gcf,'textbox',[    0.3274    0.6873    0.1124    0.0394],'Color',[0.501960784313725 0 0.501960784313725],...
    'String','-- Med (G2-st)',...
    'LineStyle','none',...
    'FontWeight','bold',...
    'FontSize',14,...
...
annotation(gcf,'textbox',[   0.3267    0.6570    0.0975    0.0381],'Color',[0 0 1], ...
    'String','-- Con G2-st)',...
    'LineStyle','none',...
    'FontWeight','bold',...
    'FontSize',14,...
...
%---------------------------------------------------
annotation(gcf,'textbox',[ 0.8044    0.5965    0.2669    0.3342],'String',{'Stim-Induced','       (M2)'}','Rotation',90,'FontWeight','bold','FontSize',annotationFontSize,'FitBoxToText','off',...
    'EdgeColor','none');

% Create textbox
annotation(gcf,'textbox',[0.8044    0.1866    0.2669    0.3342],'String',{'Spontaneous','    (M2-G2)'},'Rotation',90,'FontWeight','bold','FontSize',annotationFontSize,'FitBoxToText','off',...
    'EdgeColor','none');
