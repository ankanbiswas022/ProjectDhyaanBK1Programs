function [imdata] = make_grating_s4_o135(TrialRecord,MLConfig)
...
%                  movies.
%   info.TimePerFrame: This field sets the intervals of the frames (in sec)
...
if isfield(TrialRecord.CurrentConditionInfo,'sf2')
    SpatialFrequency = TrialRecord.CurrentConditionInfo.sf2;
else
    SpatialFrequency = 45;      % deg
end

if isfield(TrialRecord.CurrentConditionInfo,'ori4')
    Orientation = TrialRecord.CurrentConditionInfo.ori4;
else
    Orientation = 135;      % deg
end

ApertureRadius = 31;        % deg
% SpatialFrequency = 1;      % cycles per deg
TemporalFrequency = 0;     % cycles per sec
...
% for m=1:length(Time)
    grating = (sine_grating(x,y,Orientation,SpatialFrequency,TemporalFrequency,0) + 1) / 2;  % [-1 1] to [0 1]
    imdata(:,:,:) = cat(3,alpha,grating,grating,grating);  % [a r g b]
% end

% info.TimePerFrame = 1/RefreshRate;  % Inform that the frame interval of this movie is 0.016667 sec.
